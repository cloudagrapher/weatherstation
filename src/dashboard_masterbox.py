...
class WeatherDashboard:
    ...
    def get_pressure_trend(self, hours=1):
        ...
        pressure_change = (
            recent_with_pressure[-1]["pressure_hpa"]
            - recent_with_pressure[0]["pressure_hpa"]
        )
    ...
    def detect_current_conditions(self):
        ...
        if pressure:
            ...
            if len(pressure_readings) >= 2:
                pressure_change_rate = (
                    pressure_readings[-1]["pressure_hpa"]
                    - pressure_readings[0]["pressure_hpa"]
                ) / 1.0
        ...
        if humidity >= 92:
            ...
            if len(recent_10min) >= 3:  # At least 3 readings in 10 minutes
                high_humidity_count = sum(
                    1 for r in recent_10min if r.get("humidity", 0) >= 92
                )
                humidity_sustained = (
                    high_humidity_count >= len(recent_10min) * 0.7
                )  # 70% of readings
        ...
        if humidity >= 92 and (pressure_change_rate <= -1.5 or humidity_sustained):
            current_conditions.append(
                "🌧️ HEAVY PRECIPITATION LIKELY/ONGOING - High humidity with pressure fall or sustained moisture"
            )
        elif humidity > 85:
            current_conditions.append(
                "🌦️ LIGHT PRECIPITATION POSSIBLE - Very high humidity"
            )
        ...
        if temp_f >= 82 and dew_point_f >= 66 and pressure_change_rate <= -1.5:
            current_conditions.append(
                "⛈️ THUNDERSTORM LIKELY (2-6h) - Heat + moisture + pressure fall"
            )
        # Alternative rule: T ≥ 78°F, dewpoint ≥ 70°F, spread ≤ 12°F
        elif temp_f >= 78 and dew_point_f >= 70 and temp_dew_spread <= 12:
            current_conditions.append(
                "⚡ THUNDERSTORM POSSIBLE - High dewpoint with low spread"
            )
        # Bonus signals: brief RH spike or pressure volatility
            ...
            if len(recent_30min) >= 2:
                humidity_change = recent_30min[-1].get("humidity", 0) - recent_30min[
                    0
                ].get("humidity", 0)
            ...
            if humidity_spike:
                current_conditions.append(
                    "⚡ THUNDERSTORM POSSIBLE - Rapid moisture increase detected"
                )
        ...
        if len(recent_readings) >= 3:
            ...
            if len(pressure_values) >= 3:
                ...
                if pressure_range > 2:
                    current_conditions.append(
                        "🌪️ PRESSURE VOLATILITY - Active weather system present"
                    )
    ...
    def predict_fog(self):
        ...
        if temp_dewpoint_spread <= 2:
            fog_score += 40
            fog_conditions.append(
                f"Dew point very close ({temp_dewpoint_spread:.1f}°F spread)"
            )
        elif temp_dewpoint_spread <= 4:
            fog_score += 20
            fog_conditions.append(
                f"Dew point close ({temp_dewpoint_spread:.1f}°F spread)"
            )
    ...
    def predict_weather(self):
        ...
        if predictions and confidence_scores:
            ...
            for signal_type, confidence in confidence_scores.items():
                ...
                if (
                    signal_type
                    in ["storm", "thunderstorm", "deteriorating", "storm_clearing"]
                    and confidence >= 75
                ):
            ...
            if severe_signals:
                ...
                final_confidence = max(
                    severe_combined * 100, 75
                )  # At least 75% for severe
            elif routine_signals:
                # Weighted average for routine signals
                weights = [
                    2.0 if signal_type in ["pressure_change", "dewpoint"] else 1.0
                    for signal_type in confidence_scores.keys()
                ]
                weighted_sum = sum(
                    conf * weight
                    for conf, weight in zip(
                        routine_signals, weights[: len(routine_signals)]
                    )
                )
                weight_total = sum(weights[: len(routine_signals)])
                final_confidence = (
                    (weighted_sum / weight_total) * 100 if weight_total > 0 else 50
                )
            else:
                final_confidence = 50
    ...
    def _get_overall_comparison_status(self, comparison):
        ...
        statuses = [
            comp.get("status", "unknown") for comp in comparison.get("comparisons", [])
        ]
    ...
    try:
        ...
        if end_str:
            try:
                ...
                return (
                    jsonify({"error": "Invalid end date format. Use YYYY-MM-DD"}),
                    400,
                )
...
def api_predictions():
    ...
    try:
        ...
        # Get predictions
        predictions = dashboard.data_service.get_historical_predictions(
            start_date, end_date
        )
